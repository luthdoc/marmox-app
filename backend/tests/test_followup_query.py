...
def _make_supabase_mock(return_data: list) -> MagicMock:
    """Return a Supabase mock that yields return_data for any chained query."""
    mock = MagicMock()
    mock.table.return_value.select.return_value.eq.return_value.lt.return_value.is_.return_value.execute.return_value.data = return_data
    mock.table.return_value.select.return_value.eq.return_value.lt.return_value.execute.return_value.data = return_data
    return mock
...
def test_first_followup_returns_leads_matching_all_criteria():
    ...
    mock_supabase = _make_supabase_mock(expected_leads)
...
def test_first_followup_calls_set_tenant_context_before_query():
    ...
    mock_supabase.table.side_effect = lambda name: (
        ...
        _make_supabase_mock([]).table(name),
    )[1]
...
def test_first_followup_uses_48h_threshold():
    ...
    # The lt filter on last_contact_at must use this threshold
    lt_call = mock_supabase.table.return_value.select.return_value.eq.return_value.lt
...
def test_second_followup_returns_leads_matching_all_criteria():
    ...
    mock_supabase2.table.return_value.select.return_value.eq.return_value.lt.return_value.execute.return_value.data = expected_leads
...
def test_second_followup_calls_set_tenant_context_before_query():
    """set_tenant_context deve ser chamado antes de qualquer operação no banco."""
    mock_supabase2 = MagicMock()
    mock_supabase2.table.return_value.select.return_value.eq.return_value.lt.return_value.execute.return_value.data = []
    call_order = []
...
def test_second_followup_uses_7_day_threshold():
    ...
    lt_call = mock_supabase2.table.return_value.select.return_value.eq.return_value.lt
